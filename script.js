...
function nextSlide(slideId) {
    ...
    if (slideId === 5 || slideId === '5') {
        // On letter slide: show button if letter is typed
        if (nextToReplyBtn) {
            nextToReplyBtn.style.display = letterTyped ? 'inline-block' : 'none';
        }

        // Launch confetti and start typing if needed
        if (!letterTyped) {
            launchConfetti();
            typeLetter();
        }
    } else {
        // On any other slide: hide the button
        if (nextToReplyBtn) {
...
Anh iu em nhiều lắm! 💙`;
    ...
    function type() {
        if (index < letterText.length) {
            ...
            // Show next button after letter is done
...
Anh iu em nhiều lắm! 💙`;
    ...
    const skipBtn = document.getElementById('skipTypingBtn');
    if (skipBtn) skipBtn.style.display = 'none';
    // Show next button when skipping typing
    document.getElementById('nextToReplyBtn').style.display = 'inline-block';
}
...
function showReplySection() {
    nextSlide('6'); // Chuyển sang slide trả lời lá thư
}
